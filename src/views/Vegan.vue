  ...
  <div class="vegan-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <p class="page-description">
          Delicious plant-based recipes that prove vegan food can be both nutritious and satisfying. 
          Discover creative ways to enjoy vegetables, grains, and legumes.
        </p>
      </div>
    ...
    <div class="vegan-info">
      <div class="info-cards">
        <div class="info-card">
          <el-icon class="info-icon" color="#52c41a"><Trophy /></el-icon>
          <h3>Health Benefits</h3>
          <p>Lower cholesterol, reduced inflammation, better digestion</p>
        </div>
        <div class="info-card">
          <el-icon class="info-icon" color="#1890ff"><Link /></el-icon>
          <h3>Environmental</h3>
          <p>Lower carbon footprint, reduced water usage, sustainable</p>
        </div>
        <div class="info-card">
          <el-icon class="info-icon" color="#722ed1"><StarFilled /></el-icon>
          <h3>Compassionate</h3>
          <p>Cruelty-free, ethical choices, animal-friendly</p>
        </div>
      </div>
    </div>
    ...
    <div class="results-info">
      <p>
        Showing {{ filteredRecipes.length }} of {{ veganRecipes.length }} vegan recipes
    ...
    <div class="recipes-container">
      ...
      <div v-if="filteredRecipes.length === 0" class="empty-state">
        <el-empty description="No vegan recipes found">
          <template #image>
            <el-icon size="80" color="#c0c4cc"><Orange /></el-icon>
          </template>
...
import { Orange, Grid, List, Trophy, Link, StarFilled } from '@element-plus/icons-vue'
...
const averageTime = computed(() => {
  if (veganRecipes.length === 0) return 0
  const total = veganRecipes.reduce((sum, recipe) => sum + recipe.totalTime, 0)
  return Math.round(total / veganRecipes.length)
})

const averageCalories = computed(() => {
  if (veganRecipes.length === 0) return 0
  const total = veganRecipes.reduce((sum, recipe) => sum + recipe.calories, 0)
  return Math.round(total / veganRecipes.length)
})
