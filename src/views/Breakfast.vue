<template>
  <div class="breakfast-page">
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <p class="page-description">
          Start your day right with these healthy and delicious breakfast options. 
          From quick grab-and-go meals to leisurely weekend brunches.
        </p>
      </div>
    ...
    <div class="results-info">
      <p>
        Showing {{ filteredRecipes.length }} of {{ breakfastRecipes.length }} breakfast recipes
    ...
    <div class="recipes-container">
      ...
      <div v-if="filteredRecipes.length === 0" class="empty-state">
        <el-empty description="No breakfast recipes found">
          <template #image>
            <el-icon size="80" color="#c0c4cc"><Sunrise /></el-icon>
          </template>
...
import { Sunrise, Grid, List } from '@element-plus/icons-vue'
...
const averageTime = computed(() => {
  if (breakfastRecipes.length === 0) return 0
  const total = breakfastRecipes.reduce((sum, recipe) => sum + recipe.totalTime, 0)
  return Math.round(total / breakfastRecipes.length)
})

const averageCalories = computed(() => {
  if (breakfastRecipes.length === 0) return 0
  const total = breakfastRecipes.reduce((sum, recipe) => sum + recipe.calories, 0)
  return Math.round(total / breakfastRecipes.length)
})
