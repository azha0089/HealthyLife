<template>
  <div class="vitamins-article-detail-page">
    ...
    <div v-else-if="!article" class="not-found-container">
      <el-icon size="64" color="#d9d9d9"><DocumentDelete /></el-icon>
...
// Use shared vitamin articles data  
const allVitaminArticles = vitaminArticles
...
const fetchArticle = async () => {
  ...
  const foundArticle = allVitaminArticles.find(a => a.id === articleId)
  
  if (foundArticle) {
    article.value = foundArticle
    
    // Get related articles (same category or different vitamins)
    relatedArticles.value = allVitaminArticles
      .filter(a => a.id !== foundArticle.id)
      .sort((a, b) => {
        // Prioritize same subcategory (fat-soluble vs water-soluble)
        if (a.subcategory === foundArticle.subcategory && b.subcategory !== foundArticle.subcategory) {
          return -1
        }
        if (b.subcategory === foundArticle.subcategory && a.subcategory !== foundArticle.subcategory) {
          return 1
        }
